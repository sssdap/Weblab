  ...
  return (
    <Card
      className={`transition-all duration-200 ${
        completed ? "border-success/30 bg-success/5" : "hover:shadow-lg"
      }`}
    >
      <CardHeader>
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1">
            <div className="mb-2 flex items-center gap-2">
              {completed ? (
                <span className="inline-flex items-center justify-center h-6 w-6 rounded-full bg-success text-success-foreground text-xs font-semibold">
            ...
            <CardTitle className={`text-lg ${completed ? "text-success" : ""}`}>
              {lesson.title}
            </CardTitle>
            {lesson.description && (
      ...
      try {
        ...
        if (user?.id) {
          ...
          completedSnap.forEach((doc) => {
            ...
            if (
              data.lessonId &&
              lessonIds.includes(data.lessonId) &&
              data.completed
            ) {
              ids.add(data.lessonId);
            }
          });
