...
export async function signIn(
  ...
  try {
    ...
    console.log("[AUTH SERVICE] signIn: Got ID token");

    // Создание session cookie через API
...
export async function signOut(): Promise<void> {
  try {
    ...
    // Проверяем, что cookie удалилась
    const checkResponse = await fetch("/api/auth/check-session");
    const checkData = await checkResponse.json();
    console.log(
      "[AUTH SERVICE] signOut: Session check after logout:",
      checkData,
    );
